 ...
 * dtls_transport.cc
 * ---------------------------------------------------------------------------
 * Implements the interface for Dtls transportation.
 * ---------------------------------------------------------------------------
 * Reference implementation: DtlsTransport.cc (Erizo)
 */
...
Resender::Resender(std::shared_ptr<NiceConnection> nice, unsigned int comp, const unsigned char* data, unsigned int len) : 
...
void Resender::run() {
  /* Set thread name */
  prctl(PR_SET_NAME, (unsigned long)"Resender::run");
...
DtlsTransport::DtlsTransport(MediaType med, const std::string &transport_name, bool bundle, bool rtcp_mux, TransportListener *transportListener, 
  ...
  if (isServer_){
    ...
    VLOG(3) << "CREATE CLIENT, WE ACT AS CLIENT";
  ...
  rtp_packet_total_ = 0;
  rtcp_packet_total_ = 0;
  rtp_unprotect_fail_ = 0;
  rtcp_unprotect_fail_ = 0;
...
void DtlsTransport::onNiceData(unsigned int component_id, char* data, int len, NiceConnection* nice) {
  ...
  if (DtlsTransport::isDtlsPacket(data, len)) {
    ...
    if (srtp != NULL){
      ...
      if (chead->isRtcp()){
        {
          boost::mutex::scoped_lock lock(stats_mutex_);
          rtcp_packet_total_++;
        }
        if(srtp->unprotectRtcp(unprotectBuf_, &length)<0){
          LOG(ERROR) << "unprotectRtcp failed...length=" << length;
          boost::mutex::scoped_lock lock(stats_mutex_);
          rtcp_unprotect_fail_++;
          return;
        }
      } else {
        {
        ...
        if(srtp->unprotectRtp(unprotectBuf_, &length)<0){
          LOG(ERROR) << "unprotectRtp failed...length=" << length;
          boost::mutex::scoped_lock lock(stats_mutex_);
          rtp_unprotect_fail_++;
          return;
        }
...
void DtlsTransport::write(char* data, int len) {
  ...
  VLOG(4) << "DtlsTransport::Write data_len=" << len;
...
void DtlsTransport::onHandshakeCompleted(DtlsSocketContext *ctx, std::string clientKey,std::string serverKey, std::string srtp_profile) {
  ...
  VLOG(3) << "DtlsTransport. onHandshakeCompleted...";
  std::string temp;
...
void DtlsTransport::updateIceState(IceState state, NiceConnection *conn) {
  ...
  if (state == NICE_INITIAL && this->getTransportState() != TRANSPORT_STARTED) {
  ...
  else if (state == NICE_GATHERING_DONE &&
           this->getTransportState() != TRANSPORT_GATHERED) {
    VLOG(3) << "Nice Transport Gathered.";
    ...
    if (!isServer_ && dtlsRtcp != NULL &&
        (!dtlsRtcp->started || rtcpResender->getStatus() < 0)) {
...
void DtlsTransport::processLocalSdp(SdpInfo *localSdp_) {
  ...
  localSdp_->setIsFingerprint(true);
  localSdp_->setFingerprint(getMyFingerprint());
  std::string username;
  std::string password;
...
void DtlsTransport::getNiceDataLoop(){
  /* Set thread name */
  prctl(PR_SET_NAME, (unsigned long)"getNiceDataLoop");
