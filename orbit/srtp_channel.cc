...
int SrtpChannel::protectRtp(char* buffer, int *len) {
    if (buffer == NULL) {
      ELOG_WARN("FATAL SrtpChannel::protectRtp : invalid buffer, len %d", (len != NULL) ? *len : -1);
      return -1;
    }
    if (!active_)
        return 0;
...
int SrtpChannel::unprotectRtp(char* buffer, int *len) {
    if (buffer == NULL) {
      ELOG_WARN("FATAL SrtpChannel::unprotectRtp : invalid buffer, len %d", (len != NULL) ? *len : -1);
      return -1;
    }
    if (!active_)
        return 0;
...
int SrtpChannel::protectRtcp(char* buffer, int *len) {
    if (buffer == NULL) {
      ELOG_WARN("FATAL SrtpChannel::protectRtcp : invalid buffer, len %d", (len != NULL) ? *len : -1);
      return -1;
    }
...
int SrtpChannel::unprotectRtcp(char* buffer, int *len) {
    if (buffer == NULL) {
      ELOG_WARN("FATAL SrtpChannel::unprotectRtcp : invalid buffer, len %d", (len != NULL) ? *len : -1);
      return -1;
    }
...
bool SrtpChannel::configureSrtpSession(srtp_t *session, const char* key,
    ...
    ELOG_DEBUG("set master key/salt to...");
    ...
    if (res!=0){
      ...
      ELOG_ERROR("Failed to create srtp session with...");
    }
