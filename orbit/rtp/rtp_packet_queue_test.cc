...
TEST_F(RtpPacketQueueTest, rtpPacketQueueBasicBehavior) {
  ...
  orbit::RtpHeader header;
  header.setSeqNumber(12);
  queue.pushPacket((const char *)&header, sizeof(orbit::RtpHeader));
  ...
  std::shared_ptr<orbit::dataPacket> packet = queue.popPacket();
  ...
  const orbit::RtpHeader *poppedHeader = reinterpret_cast<const orbit::RtpHeader*>(packet->data);
...
TEST_F(RtpPacketQueueTest, rtpPacketQueueCorrectlyReordersSamples) {
  ...
  for(int x = 1; x <=10; x++) {
    std::shared_ptr<orbit::dataPacket> packet = queue.popPacket(true); // override our default pop behavior so we can validate these are ordered
    const orbit::RtpHeader *poppedHeader = reinterpret_cast<const orbit::RtpHeader*>(packet->data);
...
TEST_F(RtpPacketQueueTest, rtpPacketQueueCorrectlyHandlesSequenceNumberRolloverBackwards) {
  ...
  while(x != 65529) { // has to be 29, or have have an off-by-one error
    orbit::RtpHeader header;
    header.setSeqNumber(x);
    LOG(INFO) << "x=" << x << " queue.getSize()=" << queue.getSize();

    EXPECT_EQ(count, queue.getSize());
    EXPECT_EQ(false, queue.hasData());
    x -= 1;
    count++;
    queue.pushPacket((const char *)&header, sizeof(orbit::RtpHeader));
...
TEST_F(RtpPacketQueueTest, rtpPacketQueueMakesDataAvailableOnceEnoughSamplesPushed) {
  ...
  orbit::RtpPacketQueue queue(depth, max);  // max and depth.
  queue.setTimebase(1);
