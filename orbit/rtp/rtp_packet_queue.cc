 ...
 * ---------------------------------------------------------------------------
 * Implements a the RTP packet queue system.
 * ---------------------------------------------------------------------------
 */
    ...
    lastSequenceNumberGiven_(-1), timebase_(0), depthInSeconds_(depthInSeconds), maxDepthInSeconds_(maxDepthInSeconds)
{
    if(depthInSeconds_ >= maxDepthInSeconds_) {
        ...
        depthInSeconds_ = orbit::DEFAULT_DEPTH;
        maxDepthInSeconds_ = orbit::DEFAULT_MAX;
    }
}
...
void RtpPacketQueue::pushPacket(const char *data, int length)
{
    ...
    if(lastSequenceNumberGiven_ >= 0
        && (rtpSequenceLessThan(currentSequenceNumber, (uint16_t)lastSequenceNumberGiven_ - 50)
    ...
    std::shared_ptr<dataPacket> packet(new dataPacket());
    memcpy(packet->data, data, length);
    packet->length = length;

    // let's insert this packet where it belongs in the queue.
    boost::mutex::scoped_lock lock(queueMutex_);
    std::list<std::shared_ptr<dataPacket> >::iterator it;
    ...
    while(getDepthInSeconds() > maxDepthInSeconds_){
        LOG(INFO) << "getDepthInSeconds() = " << getDepthInSeconds() << ", maxDepthInSeconds_ = " << maxDepthInSeconds_;
...
std::shared_ptr<dataPacket> RtpPacketQueue::popPacket(bool ignore_depth, bool ignore_old_packet)
{
  std::shared_ptr<dataPacket> packet = NULL;
  boost::mutex::scoped_lock lock(queueMutex_);
  if (queue_.size() > 0) {
    if (ignore_depth || getDepthInSeconds() > depthInSeconds_) {
      ...
      if(ignore_old_packet && rtpSequenceLessThan((int)header->getSeqNumber(),lastSequenceNumberGiven_)) {
        packet = NULL;
      } else {
...
std::shared_ptr<dataPacket> RtpPacketQueue::getNextPacket(bool ignore_depth)
{
    std::shared_ptr<dataPacket> packet = NULL;
    boost::mutex::scoped_lock lock(queueMutex_);
    if (queue_.size() > 0) {
