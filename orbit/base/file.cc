 ...
 * --------------------------------------------------------------------------
 * file.cc
 * -- implements the classes for reading, writting and manipulating basic files.
 *
...
util::Status File::Close(int flags) {
  ...
  return Close() ? util::Status::OK
                 : util::Status(util::error::INVALID_ARGUMENT,
                                StringPrintf("Could not close file '%s'", name_.c_str()));
...
bool File::RecursivelyCreateDir(const string& path, int mode) {
  ...
  if (Exists(path.c_str())) return false;
...
void File::RecursivelyDeleteDirOrFile(const string& name) {
    ...
    if (dir != NULL) {
      while (true) {
        ...
        if (entry_name != "." && entry_name != "..") {
          RecursivelyDeleteDirOrFile(name + "/" + entry_name);
        }
      }
    }
...
util::Status Open(const std::string& filename, const std::string& mode,
  ...
  return util::Status(util::error::INVALID_ARGUMENT,
                      StringPrintf("Could not open file '%s'", filename.c_str()));
...
util::Status GetContents(const std::string& filename, std::string* output, int flags) {
  ...
  return util::Status(util::error::INVALID_ARGUMENT,
                      StringPrintf("Could not read file '%s'", filename.c_str()));
...
util::Status WriteString(File* file, const std::string& contents, int flags) {
  ...
  return util::Status(util::error::INVALID_ARGUMENT,
                      StringPrintf("Could not write %d bytes", contents.size()));
...
util::Status SetTextProto(const std::string& filename, const google::protobuf::Message& proto,
  ...
  return util::Status(util::error::INVALID_ARGUMENT,
                      StringPrintf("Could not write proto to '%s'", filename.c_str()));
...
util::Status Delete(const std::string& path, int flags) {
  ...
  return util::Status(util::error::INVALID_ARGUMENT,
                      StringPrintf("Could not delete '%s'", path.c_str()));
