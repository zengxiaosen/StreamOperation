...
namespace erizo{
  ...
  void RtcpProcessor::analyzeFeedback(char *buf, int len) {
    LOG(INFO) << "Enter analyzeFeedback...";
        ...
        switch(chead->packettype){
          ...
          case RTCP_Receiver_PT:
            ...
            if (chead->getSourceSSRC() == rtcpSource_->getVideoSourceSSRC()){
              ...
              LOG(INFO) << "Analyzing Video RR: PacketLost " << chead->getLostPackets()
                        << ", Ratio " << (int)chead->getFractionLost()
                        << ", partNum " << (int)partNum 
                        << ", blocks " << (int)chead->getBlockCount() 
                        << ", sourceSSRC " << chead->getSourceSSRC();
            }else{
              ...
              LOG(INFO) << "Analyzing Audio RR: PacketLost " << (int)chead->getLostPackets()
                        << ", Ratio " << (int)chead->getFractionLost()
                        << ", partNum " << (int)partNum 
                        << ", blocks " << (int)chead->getBlockCount() 
                        << ", sourceSSRC " << chead->getSourceSSRC();
            ...
            if (theData->lastSr==0||theData->lastDelay < delay){
              ...
              LOG(INFO) << "Recording DLSR " << (int)chead->getDelaySinceLastSr()
                        << ", lastSR " << (int)chead->getLastSr()
                        << " last delay " << (int)theData->lastDelay
                        << ", calculated delay " << (int)delay
                        <<" for SSRC " << (int)sourceSsrc;
          ...
          case RTCP_RTP_Feedback_PT:
            ...
            LOG(INFO) << "RTP FB: Usually NACKs: " << (int)chead->getBlockCount()
                      << ", partNum " << (int)partNum;
          ...
          case RTCP_PS_Feedback_PT:
            ...
            switch(chead->getBlockCount()){
              case RTCP_PLI_FMT:
                ...
                LOG(INFO) << "PLI Message, partNum " << partNum;
                // 1: PLI, 4: FIR
                theData->shouldSendPli = true;
                break;
              case RTCP_SLI_FMT:
                ELOG_DEBUG("SLI Message");
                LOG(INFO) << "SLI Message";
                break;
              case RTCP_FIR_FMT:
                ELOG_DEBUG("FIR Message");
                LOG(INFO) << "FIR Message";
                break;
              case RTCP_AFB:
                {
                  ...
                  if (!strncmp(uniqueId,"REMB", 4)){
                    ...
                    LOG(INFO) << "Received REMB " << (int)bitrate;
                    if ((bitrate<theData->reportedBandwidth) || theData->reportedBandwidth==0){
                      ...
                      LOG(INFO) << "Should send Packet REMB, before BR " << theData->reportedBandwidth << ", will send with Br " << (int)bitrate;
                    ...
                    LOG(INFO) << "Unsupported AFB Packet not REMB";
                  }
                  break;
                }
              default:
                ELOG_WARN("Unsupported RTCP_PS FB TYPE %u",chead->getBlockCount());
                LOG(INFO) << "Unsupported RTCP_PS FB TYPE " << (int)chead->getBlockCount();
                break;
            }
            break;
          default:
            ELOG_DEBUG("Unknown RTCP Packet, %d", chead->packettype);
            LOG(INFO) << "Unknown RTCP Packet, " << (int)chead->packettype;
            break;
        }
    ...
    for (it = rtcpData_.begin(); it != rtcpData_.end(); it++){
      ...
      if((rtcpData->requestRr||dt >= rtcpData->nextPacketInMs ||rtcpData->shouldSendREMB) && rtcpData->lastSr > 0){  // Generate Full RTCP Packet
        ...
        if (sourceSsrc == rtcpSource_->getAudioSourceSSRC()){
          ...
          LOG(INFO) << "Sending Audio RR: PacketLost " << (int)rtcpData->totalPacketsLost
                    << ", Ratio " <<  (int)rtcpData->ratioLost
                    << ", DLSR " << (int)rtcpData->delaySinceLastSr
                    << ", lastSR " << (int)rtcpData->lastSr
                    << ", DLSR Adjusted " << (int)(rtcpData->delaySinceLastSr+edlsr)
                    << " dt " << dt
                    << ", rrsSinceLast " << (int)rtcpData->rrsReceivedInPeriod 
                    << " highestSeqNum " << (int)rtcpData->highestSeqNumReceived;
          ...
          LOG(INFO) << "Sending Audio RR: SOurcessrc" << (int)sourceSsrc
                    << "sinkSsrc" << (int)rtcpSink_->getVideoSinkSSRC()
                    << "PacketLost " << (int)rtcpData->totalPacketsLost
                    << ", Ratio " <<  (int)rtcpData->ratioLost
                    << ", DLSR " << (int)rtcpData->delaySinceLastSr
                    << ", lastSR " << (int)rtcpData->lastSr
                    << ", DLSR Adjusted " << (int)(rtcpData->delaySinceLastSr+edlsr)
        ...
        if (rtcpData->mediaType == VIDEO_TYPE){
          ...
          if(rtcpData->shouldSendREMB ){
            ...
            LOG(INFO) << "Sending Remb, since last " << sincelastREMB << " ms, sending with BW: " << rtcpData->reportedBandwidth;
        ...
        if ( rtcpData->mediaType == AUDIO_TYPE){
          ...
          LOG(INFO) << "Scheduled next Audio RR in " << rtcpData->nextPacketInMs << " ms";
        } else {
          rtcpData->nextPacketInMs = (RR_VIDEO_BASE)*random;
          ELOG_DEBUG("Scheduled next Video RR in %u ms", rtcpData->nextPacketInMs);
          LOG(INFO) << "Scheduled next Video RR in " << rtcpData->nextPacketInMs << " ms";
        }
  ...
  int RtcpProcessor::addNACK(char* buf, int len, uint16_t seqNum, uint16_t blp, uint32_t sourceSsrc, uint32_t sinkSsrc){
    ...
    LOG(INFO) << "Setting PID " << (int)seqNum << " BLP " << (int)blp;
    RtcpHeader theNACK;
