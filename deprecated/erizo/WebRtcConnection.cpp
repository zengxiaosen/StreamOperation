...
namespace erizo {
  ...
  void WebRtcConnection::close() {
    // HACK(chengxu): do nothing... this is a very hacky way to close
    // a connection.
  }
  ...
  bool WebRtcConnection::processOffer(const std::string& offer_sdp,
                                      std::string* answer_sdp) {
    // HACK(chengxu): try the remoteSdp same as local_sdp
    LOG(INFO) << "processOffer...";
    bool ret = setRemoteSdp(offer_sdp);
    if (ret) {
      localSdp_.videoSsrc = remoteSdp_.videoSsrc;
      localSdp_.audioSsrc = remoteSdp_.audioSsrc;
      localSdp_.videoRtxSsrc = remoteSdp_.videoRtxSsrc;
      //localSdp_.videoSsrc = -1;
      //localSdp_.audioSsrc = -1;
      //localSdp_.videoRtxSsrc = -1;
      *answer_sdp = getAnswer();
      return true;
    } else {
      return false;
    }
  ...
  bool WebRtcConnection::setRemoteSdp(const std::string &sdp) {
    ...
    if (remoteSdp_.profile == SAVPF) {
      if (remoteSdp_.isFingerprint) {
        if (remoteSdp_.hasVideo||bundle_) {
          ...
          if (!videoTransport_){
            VLOG(3) << "Creating videoTransport with creds (" << username.c_str() << "," << password.c_str() << ")";
            ELOG_DEBUG("Creating videoTransport with creds %s, %s", username.c_str(), password.c_str());
            bool isServer = false;
            videoTransport_ = new DtlsTransport(VIDEO_TYPE, "video", bundle_, remoteSdp_.isRtcpMux, this, iceConfig_ , username, password, isServer);
          }else{ 
            VLOG(3) << "UPDATING videoTransport with creds (" << username.c_str() << "," << password.c_str() << ")";
        ...
        if (!bundle_ && remoteSdp_.hasAudio) {
          ...
          if (!audioTransport_){
            VLOG(3) << "Creating audioTransport with creds (" << username.c_str() << "," << password.c_str() << ")";

            ELOG_DEBUG("Creating audioTransport with creds %s, %s", username.c_str(), password.c_str());
            bool isServer = false;
            audioTransport_ = new DtlsTransport(AUDIO_TYPE, "audio", bundle_, remoteSdp_.isRtcpMux, this, iceConfig_, username, password, isServer);
          }else{
            VLOG(3) << "UPDATING audioTransport with creds (" << username.c_str() << "," << password.c_str() << ")";
    ...
    VLOG(3) << "Before trickle enabled....";
    
    if(trickleEnabled_){
      std::string object = this->getLocalSdp();
      VLOG(3) << "object=" << object;
    ...
    VLOG(3) << "See the remote sdp has candidate.......";
  ...
  std::string WebRtcConnection::getAnswer() {
    std::string localSdp = getLocalSdp();
    return localSdp;
  }
  ...
  void WebRtcConnection::onTransportData(char* buf, int len, Transport *transport) {
    ...
    VLOG(3) << "onTransportData...len=" << len;
    ...
    if (chead->isRtcp()) {
      ...
      if (chead->packettype == RTCP_Sender_PT) { //Sender Report
        LOG(INFO) << "rtcpProcessor_->analyzeSr()";
    ...
    if (chead->isFeedback()){
      VLOG(3) << "This is a feedback packets.....";
      if (fbSink_ != NULL && shouldSendFeedback_) {
        LOG(INFO) << "This is a feedback packets.....";
      ...
      if (bundle_) {
        ...
        if (recvSSRC==this->getVideoSourceSSRC()) {
          ...
          VLOG(3) << "1.deliverVideoData len=" << len << " videoSink_=" << videoSink_;
          videoSink_->deliverVideoData(buf, len);
        } else if (recvSSRC==this->getAudioSourceSSRC()) {
          parseIncomingPayloadType(buf, len, AUDIO_PACKET);
          VLOG(3) << "1.deliverAudioData len=" << len << " audioSink_=" << audioSink_;
          ...
          LOG(ERROR) << "Unknown SSRC " << recvSSRC << ", localVideo " << this->getVideoSourceSSRC() << ", remoteVideo " << this->getVideoSinkSSRC() << ", ignoring";
        }
        ...
        if (audioSink_ != NULL) {
          ...
          VLOG(3) << "deliverAudioData len=" << len;
          audioSink_->deliverAudioData(buf, len);
        }
        ...
        if (videoSink_ != NULL) {
          ...
          VLOG(3) << "deliverVideoData len=" << len;
          videoSink_->deliverVideoData(buf, len);
        }
      }
    }
...
//            ELOG_ERROR("onTransportData did not find mapping for %i", externalPT);
  ...
  void WebRtcConnection::queueData(int comp, const char* buf, int length, Transport *transport, packetType type) {
    ...
    VLOG(3) << "sending queue.... sendQueue_.size()=" << sendQueue_.size()
              << " buf_length=" << length;
    ...
    int  queue_remains = 0;
      while (sending_) {
          ...
          if (bundle_ || p.type == VIDEO_PACKET) {
            ...
            VLOG(3) << "Write to transport Video: p.length=" << p.length << " queue_remains=" << queue_remains;

            if (p.type == VIDEO_PACKET) {
              VLOG(3) << "dataPacket.length=" << p.length;
              VLOG(3) << "dataPacket.comp=" << p.comp;
              RtpHeader* rtp = reinterpret_cast<RtpHeader*>(&(p.data[0]));
              VLOG(3) << "ssrc=" << rtp->getSSRC();
              //if (rtp->getSSRC() == this->videoSourceSSRC_) {
                VLOG(3) << "set to 55543...yeah";
                rtp->setSSRC(55543);
                //}
            }
            ...
            VLOG(3) << "Write to transport Audio: p.length=" << p.length << " queue_remains=" << queue_remains;
