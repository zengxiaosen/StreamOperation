...
#include "stream_service/erizo/rtp/RtpVP8Parser.h"
    ...
    firstVideoTimestamp_(-1), firstAudioTimestamp_(-1), firstDataReceived_(-1), videoOffsetMsec_(-1), audioOffsetMsec_(-1), vp8SearchState_(lookingForStart), needToSendFir_(true)
{
    ...
    if (context_==NULL){
        ...
        LOG(INFO) << "context_->oformat" << context_->oformat;
        if (!context_->oformat){
    ...
    LOG(INFO) << "filename=" << context_->filename << " oformat=" << context_->oformat;
...
void ExternalOutput::writeVideoData(char* buf, int len){
    ...
    LOG(INFO) << "Parsed VP8 payload, endOfFrame: " << endOfFrame << ", startOfFrame: " << startOfFrame << ", currentVideoSeqNumber: " << currentVideoSeqNumber;
    this->initContext();
...
int ExternalOutput::deliverAudioData_(char* buf, int len) {
  LOG(INFO) << "ExternalOutput:deliverAudioData_...len=" << len;
    this->queueData(buf,len,AUDIO_PACKET);
    return 0;
}

int ExternalOutput::deliverVideoData_(char* buf, int len) {
  LOG(INFO) << "ExternalOutput:deliverVideoData_...len=" << len;
    ...
    LOG(INFO) << "initContext:" << context_->oformat->audio_codec;
      ...
      AVCodec* videoCodec = avcodec_find_encoder(context_->oformat->video_codec);
        ...
        if (audioCodec==NULL){
            ...
            LOG(FATAL) << "Could not find audio codec";
            return false;
        }
...
void ExternalOutput::queueData(char* buffer, int length, packetType type){
    ...
    LOG(INFO) << "ExternalOutput::queueData length=" << length;

    RtcpHeader *head = reinterpret_cast<RtcpHeader*>(buffer);
    if (head->isRtcp()){
      LOG(INFO) << "this is RTcp packet. Return.";
        return;
    }
    ...
    if (type == VIDEO_PACKET){
        if(this->videoOffsetMsec_ == -1) {
            ...
            LOG(INFO) << "File " << context_->filename << ", video offset msec:" << videoOffsetMsec_;
        }
        ...
        if (h->getPayloadType() == RED_90000_PT) {
          ...
          LOG(INFO) << "videoQueue pushPacket....";
    ...
    if( audioQueue_.hasData() || videoQueue_.hasData()) {
      ...
      LOG(INFO) << "Notify to write stuff...";
        cond_.notify_one();
    }
}
...
void ExternalOutput::sendLoop() {
  while (recording_) {
    ...
    while (audioQueue_.hasData()) {
      ...
      LOG(INFO) << "writeAudioData-" << audioP->length;
    }
    while (videoQueue_.hasData()) {
      ...
      LOG(INFO) << "writeVideoData-" << videoP->length;
    }
