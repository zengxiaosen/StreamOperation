...
namespace erizo {
  ...
  int ExternalInput::init(){
    ...
    if(res != 0){
      ...
      LOG(ERROR) << "Error opening input err=" << errbuff;
      return res;
    }
    ...
    if(res<0){
      ...
      LOG(ERROR) << "Error finding stream err=" << errbuff;
      return res;
    }
    ...
    if (st->codec->codec_id==AV_CODEC_ID_VP8 || !om.hasVideo){
      ...
      LOG(INFO) << "No need for video transcoding, already VP8";
  ...
  void ExternalInput::receiveLoop(){
    ...
    LOG(INFO) << "Start playing external input " << url_.c_str();
    while(av_read_frame(context_,&avpacket_)>=0&& running_==true){
      ...
      if (needTranscoding_){
        ...
        if(avpacket_.stream_index == video_stream_index_){//packet is video
          ...
          LOG(INFO) << "package_video.... av_packet_.size=" << avpacket_.size;
          ...
          LOG(INFO) << "package_audio.... av_packet_.size=" << avpacket_.size;
